...
export default function ChatScreen() {
  ...
  // Load initial messages via REST; socket handles live updates
  const { data, isLoading } = useQuery({
    queryKey: ['chat-messages', id],
    queryFn: () => chatService.getMessages(id),
    staleTime: Infinity, // socket keeps data fresh
    enabled: !!id,
  });
  ...
  // Merge REST messages + live socket messages, deduplicate by id
  const messages = [
    ...baseMessages,
    ...liveMessages.filter(m => !baseMessages.find((b: any) => b.id === m.id)),
  ];
  ...
  // Socket.io: join room, receive live messages & typing indicator
  useEffect(() => {
    if (!id) return;
    socketService.connect().then(() => {
      socketService.joinChat(id);
      socketService.onNewMessage((msg: any) => {
        if (msg.chat_id === id) {
          setLiveMessages(prev => [...prev, msg]);
        }
      });
      socketService.onTyping(({ chatId, userId }: any) => {
        if (chatId === id && userId !== user?.id) {
          setTypingUser(true);
          if (typingTimer.current) clearTimeout(typingTimer.current);
          typingTimer.current = setTimeout(() => setTypingUser(false), 3000);
        }
      });
    });
    return () => {
  ...
  // Send via socket (optimistic) + fallback REST mutation
...
const styles = StyleSheet.create({
  ...
  typingText: { fontSize: 11, color: COLORS.primary, fontStyle: 'italic', marginTop: 1 },
